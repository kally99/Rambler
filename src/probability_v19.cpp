...
double dnorm1(double x, double mean, double sd, bool return_log) {
  return R::dnorm4(x, mean, sd, return_log);
}
...
double dlnorm1(double x, double meanlog, double sdlog, bool return_log) {
  return R::dlnorm(x, meanlog, sdlog, return_log);
}

//------------------------------------------------
// draw from gamma distribution with given shape (alpha) and rate (beta)
double rgamma1(double shape, double rate) {
  return R::rgamma(shape, 1.0 / rate);
}

//------------------------------------------------
// density of gamma distribution with given shape (alpha) and rate (beta)
double dgamma1(double x, double shape, double rate, bool return_log) {
  return R::dgamma(x, shape, 1.0 / rate, return_log);
}

//------------------------------------------------
// draw from inverse gamma distribution with given shape (alpha) and scale
// (beta)
...
//------------------------------------------------
// density of inverse gamma distribution with given shape (alpha) and scale
// (beta)
double dinvgamma1(double x, double shape, double scale, bool return_log) {
  double ret = -2*log(x) + R::dgamma(1.0 / x, shape, 1.0 / scale, true);
  if (!return_log) {
    ret = exp(ret);
  }
  return ret;
