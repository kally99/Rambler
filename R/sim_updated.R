...
#'
#' @param samp_time list of vector of times at which samples were sequenced for each individual.
...
sim_ind <- function(samp_time, haplo_freqs, lambda, decay_rate, sens) {
  ...
  t_inf <- sort(runif(n_inf, samp_time[1], samp_time[length(samp_time)]))
  t_end <- t_inf
  
  # define a matrix for holding the intermediate information for each allele, 
  # including the baseline allele information
  state_haplo <- matrix(NA, n_haplo, n_inf + 1)
  ...
  for (j in 1:n_haplo) {
    ...
    if (init_infected) {
      t_decay <- samp_time[1] + rexp(1, rate = decay_rate)
      state_true[j, (samp_time < t_decay)] <- 1
      state_haplo[j, 1] <- t_decay
    }
  }
...
sim_cohort <- function(n, samp_time, haplo_freqs, lambda, decay_rate, sens) {
  ...
  raw_list <- list()
  for (i in 1:n) {
    raw_list[[i]] <- sim_ind(samp_time[[i]], haplo_freqs, lambda[i], decay_rate, sens)
  }
  
  # get observed data into data.frame
  max.row <- max(lengths(samp_time))
  
  df_data <- mapply(function(i) {
    data.frame(ind = i,
               haplo = seq_along(haplo_freqs),
               time = rep(samp_time[[i]], each = length(haplo_freqs)),
